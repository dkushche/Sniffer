...
void                    init_daemon( void )
{
	...
	if (daemon.sock_raw < 0)
		sock_error("Trouble with creating socket", daemon.status, 1);
...
static void             read_chanel(t_sniffer *this)
{
    ...
    if ( status < 0 && errno == EAGAIN)
        ...
        sock_error("Reading from chanel", this->status, 1);
    else if (status > 0)
    {
...
static void             processing_request(t_sniffer *this, unsigned char cmd)
{
    ...
    if (cmd == START)
    {
        ...
        { sock_error("Trouble with reading from chanel", this->status, 0); return ;}
        if (!(pack = searcher(&this->data, buf)))
            sock_error("No packages from this address", this->status, 0);
        ...
        sock_error("Incorect client request", this->status, 0);
        stat = 0;
        write(this->out_chan, &stat, sizeof(unsigned char));
    }
}
...
static void             gen_fifo(t_sniffer *this)
{
    ...
    if (fifo < 0 && errno != EEXIST)
        sock_error("Creating FIFO", this->status, 1);
    ...
    if (this->in_chan < 0 || this->out_chan < 0)
        sock_error("Opening chanels", this->status, 1);
...
static void             sniff_time( t_sniffer *daemon, unsigned char *buf )
{
    ...
    if (recvfrom(daemon->sock_raw, buf, BUFF_SIZE, 0, &saddr, &saddr_len) < 0)
        sock_error("Reciving error", daemon->status, 1);
